...
get_cor_data <- function(dropout_index, seed_value) {
    ...
    data <- read.table(
        file = paste0("imputation_magic_data/magic_", dropout_index, "_", seed_value, ".csv"),
        header = FALSE,
        sep = ","
    )
    ...
    data_drimpute = readRDS(paste0("imputation_drimpute_data/drimpute_",dropout_index, "_",seed_value,".rds"))
    ...
    data_our = readMat(paste0("imputation_our_data/our_",dropout_index, "_seed_",seed_value,".mat"))[[1]]
...
plot_cell_distribution <- function(dropout_index, seed_value) {
    methods <- c(
        ...
        "MAGIC", "PBLR", "SAVER", "scImpute", "scTSSR", "scVI"
    )
    # load the simulationd data
    data_simulation <- readRDS(file = paste0(
        "simulation_data/simulation_data_dropout_index_",
    ...
    #saveRDS(data_cor, paste0("D:/MyWorkWorld/Imputation/scWMC/simulation_2/cor/", dropout_index, "_", seed_value, ".rds"))
    
    data_cor = readRDS("D:/MyWorkWorld/Imputation/scWMC/simulation_2/cor/1_1.rds")
    ...
    for (j in group_info) {
        ...
        p[[k]] <- ggplot(data_bar, aes(x = x, y = y, fill = methods)) +
            ...
            theme(
                plot.title = element_text(face = "bold", size = 12, hjust = 0.4),
                axis.text.x = element_text(angle = -30, size = 3, face = "bold"),
                legend.position = "bottom",
                panel.grid.major = element_blank(),
                panel.grid.minor = element_blank(),
                panel.background = element_blank()
...
plot_gene_distribution <- function(dropout_index, seed_value) {
    ...
    data_cor <- readRDS(paste0("D:/MyWorkWorld/Imputation/scWMC/simulation_2/cor/", dropout_index, "_", seed_value, ".rds"))
