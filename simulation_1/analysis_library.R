...
generate_save_data <- function(dropout_index, seed_value){
  ...
  filename <- paste('simulation_data/simulation_data_dropout_index_',dropout_index, 
                    '_seed_', seed_value, '.mat', sep = "")
  writeMat(filename, data_bulk=data_simulation$data_bulk, data_dropout=data_simulation$data_dropout, 
...
######## SAVER #################
run_saver <-function(dropout_index, seed_value){
  library(SAVER)
  # load the data
  data = readRDS(file = paste0('simulation_data/simulation_data_dropout_index_',
                               dropout_index,
  ...
  out <- saver(data_dropout, do.fast = TRUE, ncores = 6)

  write.table(out$estimate,
              paste0(path, "saver_",dropout_index,"_",seed_value,".csv"),
              sep=',',
              row.names = F,
              col.names = F)
...
######## scTSSR #################
run_scTSSR <-function(dropout_index, seed_value){
  ...
  out <- scTSSR(data_dropout, ncores = 6, estimates.only = TRUE)

  write.table(out,
              paste0(path, "scTSSR_",dropout_index,"_",seed_value,".csv"),
              sep=',',
              row.names = F,
              col.names = F)
...
run_error <- function(dropout_index, seed_value){
  ...
  data_magic = read.table(file = paste0("imputation_magic_data/magic_",dropout_index,"_seed_",seed_value,".csv"),
                  header = FALSE,
                  sep = ",")
  # browser()
  ...
  data_our = read.table( file = paste0("imputation_our_data/our_",
  ...
  error[7] = norm(log10(data_our + 1) - log10(data_true + 1), type = "2")
  
  error[8] = data_simulation$percentage_zeros
  
  # calulate the similarity between the cell-cell
  # correlation of the imputed data and the one of the true data
  
  error_cell = matrix(0, nrow = 8, ncol = 1)
  ...
  error_cell[7] = calculate_similarity(data_true_cell, data_our_cell)
  
  error_cell[8] = data_simulation$percentage_zeros
  
  
  # calulate the similarity between the gene-gene
  # correlation of the imputed data and the one of the true da
  error_gene = matrix(0, nrow = 8, ncol = 1)
  ...
  error_gene[7] = calculate_similarity(data_true_gene, data_our_gene)
  
  error_gene[8] = data_simulation$percentage_zeros
...
plot_comparison <- function(data, ylabels = "Error", ylim_value = 100, h_ylim = 10){
  ...
  # calculate the dropout rate
  dropout_rate = round(mean(data[8,]))
  
  # the number of data using for plotting the boxplot
  # dataV1 is built with two columns: y values and group labels
  N = dim(dataV1)[1]     
  
  dataV1$group = rep(c(1:7), each = N/7)
  
  colnames(dataV1) = c('y','group')
  
  # define the comparison lists
  my_comparisons <- list( c("1", "7"), c("2", "7"), c("3", "7"), c("4", "7"), c("5", "7"), c("6", "7"))
  
  # compare the errors using t-test
  pval <- compare_means(y ~ group,data = dataV1, method = "t.test", ref.group = "7", paired = TRUE)
  
  # plot the boxplot with pvalues for the comparisons
  pp <- ggboxplot(dataV1, x = "group", y = "y", fill = "group",
                  ) +
    stat_boxplot(geom = "errorbar", width = 0.3) +
    ylim(c(0,ylim_value + 6*h_ylim)) + 
    theme_bw() +
    geom_signif(comparisons = my_comparisons, 
                annotations = formatC(pval$p, format = "e", digits = 2),
                tip_length = 0.01,
                y_position = c(ylim_value + 5*h_ylim, ylim_value + 4*h_ylim, ylim_value + 3*h_ylim, ylim_value + 2*h_ylim, ylim_value + h_ylim, ylim_value)) +
    theme(text=element_text(size=14)) +
    xlab("Method") + 
    ylab(ylabels) + 
    ggtitle(paste0("Dropout Rate: ",dropout_rate,"%")) +
    scale_fill_manual( values = c("#00AFBB","#0000CD", "#E7B800", "#FC4E07",  "#ef8a62", "#6ebb00", "#FF69B4"),
                       name="Method",
                       breaks=c("1", "2", "3", "4", "5", "6", "7"),
                       labels=c("Dropout", "DrImpute", "scImpute", "MAGIC", "VIPER", "SCRABBLE", "our")) +
...
plot_meansd <- function(dropout_index, seed_value){
  ...
  # load imputed data from our
  data_our = read.table( file = paste0("imputation_our_data/our_", dropout_index, "_seed_", 
                              seed_value,
                              ".csv"),
                          header = FALSE, sep=",")

  data_our = as.matrix(data_our)                        
  
  ratio_zeros_true <- 1 - nnzero(data_true)/length(data_true)
  ...
  p = meanSdPlot(log10(data_our + 1), ranks = FALSE)
  
  pl[[8]] = p$gg + ggtitle("our")
  
  # combine the six plots as a whole one
  main = grid.arrange(grobs = pl,ncol = 8)
...
# load the MAGIC imputed data 
  ...
  # our
  pl[[7]] = prepare_ma_data_plot(data_true, as.matrix(data_our), "our")
...
get_cor_data <- function(dropout_index, seed_value){
  ...
  data = read.table(file = paste0("imputation_magic_data/magic_",dropout_index,"_seed_",seed_value,".csv"),
                  header = FALSE,
                  sep = ",")
...
plot_cell_distribution <- function(dropout_index, seed_value){
  ...
  methods = c("True Data", "Dropout Data", "DrImpute", "scImpute", "MAGIC", "VIPER", "SCRABBLE", "our")
  # load the simulationd data
  data_simulation = readRDS(file = paste0('simulation_data/simulation_data_dropout_index_',
                                           dropout_index,
                                           '_seed_',
