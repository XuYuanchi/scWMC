...
cal_metric <- function(count){
  ...
  label <- as.integer(factor(colnames(count)))
  ...
  jaccard <- mclustcomp(label, as.integer(x.seurat$seurat_clusters), types = "jaccard")$scores
...
for (j in c(1:10)){
  # ARI = readRDS(paste0(dir, "Re/Seurat/ARI_", j, ".rds"))
  # NMI = readRDS(paste0(dir, "Re/Seurat/NMI_", j, ".rds"))
  # jaccard = readRDS(paste0(dir, "Re/Seurat/Ja_", j, ".rds"))
for(i in c(3:6)) {
  ...
  # MAGIC
  data = t(read.table(MAGIC_file, header = TRUE, sep = ",", row.names = 1))
  rownames(data) = gene
  colnames(data) = label
  metric = cal_metric(data)
  ARI[7, i] = metric[1]
...
# save data
saveRDS(ARI, file = paste0(dir, "Re/Seurat/ARI_", j, ".rds"))
saveRDS(NMI, file = paste0(dir, "Re/Seurat/NMI_", j, ".rds"))
saveRDS(jaccard, file = paste0(dir, "Re/Seurat/Ja_", j, ".rds"))
}